...
export default function TechniciansPage() {
  ...
  const filtered = technicians?.filter(t =>
    [t.full_name, t.email, t.employee_code].filter(Boolean).some(f => f!.toLowerCase().includes(search.toLowerCase()))
  ) ?? [];

  return (
    <div className="animate-fade-in">
      <PageHeader title="Monteurs" description="Beheer veldmonteurs en hun toewijzingen"
        action={<Button onClick={() => navigate('/technicians/new')}><Plus className="mr-2 h-4 w-4" /> Monteur Toevoegen</Button>} />

      {isLoading ? (
        <div className="flex justify-center py-12"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary" /></div>
      ) : technicians?.length === 0 ? (
        <EmptyState icon={HardHat} title="Nog geen monteurs" description="Voeg monteurs toe om ze aan projecten te koppelen."
          action={<Button variant="outline" onClick={() => navigate('/technicians/new')}><Plus className="mr-2 h-4 w-4" /> Monteur Toevoegen</Button>} />
      ) : (
        <>
          <ListToolbar searchValue={search} onSearchChange={setSearch} searchPlaceholder="Monteurs zoeken..." />
              ...
              <Card key={tech.id} className="cursor-pointer hover:border-primary/30 transition-colors" onClick={() => navigate(`/technicians/${tech.id}`)}>
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-1">
                    <div><p className="text-sm font-medium text-foreground">{tech.full_name}</p>
                      {tech.employee_code && <p className="text-xs text-muted-foreground font-mono">{tech.employee_code}</p>}</div>
          ...
          {filtered.length === 0 && search && <p className="text-sm text-muted-foreground text-center py-8">Geen monteurs gevonden voor "{search}"</p>}
        </>
      )}
    </div>
  );
