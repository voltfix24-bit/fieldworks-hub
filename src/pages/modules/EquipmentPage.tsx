...
export default function EquipmentPage() {
  ...
  const filtered = equipment?.filter(e =>
    [e.device_name, e.brand, e.model, e.serial_number].filter(Boolean).some(f => f!.toLowerCase().includes(search.toLowerCase()))
  ) ?? [];

  return (
    <div className="animate-fade-in">
      <PageHeader title="Apparatuur" description="Beheer meetinstrumenten en kalibratiestatus"
        action={<Button onClick={() => navigate('/equipment/new')}><Plus className="mr-2 h-4 w-4" /> Apparaat Toevoegen</Button>} />

      {isLoading ? (
        <div className="flex justify-center py-12"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary" /></div>
      ) : equipment?.length === 0 ? (
        <EmptyState icon={Wrench} title="Geen apparatuur geregistreerd" description="Registreer meetinstrumenten voor tracking en kalibratie."
          action={<Button variant="outline" onClick={() => navigate('/equipment/new')}><Plus className="mr-2 h-4 w-4" /> Apparaat Toevoegen</Button>} />
      ) : (
        <>
          <ListToolbar searchValue={search} onSearchChange={setSearch} searchPlaceholder="Apparatuur zoeken..." />
          <div className="hidden md:block">
            <Card><CardContent className="p-0">
              <table className="w-full"><thead><tr className="border-b border-border">
                <th className="text-left text-xs font-medium text-muted-foreground px-4 py-3">Apparaat</th>
                <th className="text-left text-xs font-medium text-muted-foreground px-4 py-3">Merk / Model</th>
                <th className="text-left text-xs font-medium text-muted-foreground px-4 py-3">Serienr.</th>
                <th className="text-left text-xs font-medium text-muted-foreground px-4 py-3">Kalibratie</th>
                <th className="text-left text-xs font-medium text-muted-foreground px-4 py-3">Volgende Kal.</th>
                <th className="text-left text-xs font-medium text-muted-foreground px-4 py-3">Status</th>
              </tr></thead>
              <tbody>{filtered.map(eq => (
                <tr key={eq.id} className="border-b border-border last:border-0 hover:bg-muted/50 cursor-pointer transition-colors" onClick={() => navigate(`/equipment/${eq.id}`)}>
                  <td className="px-4 py-3"><div className="flex items-center gap-2">
                    ...
                    {eq.is_default && <Badge variant="outline" className="text-[10px] px-1.5 py-0 border-tenant-primary text-tenant-primary"><Star className="h-3 w-3 mr-0.5" />Standaard</Badge>}
                  </div></td>
                ...
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-1">
                    <div><div className="flex items-center gap-2">
                      ...
                      {eq.is_default && <Badge variant="outline" className="text-[10px] px-1.5 py-0"><Star className="h-3 w-3 mr-0.5" />Standaard</Badge>}
                    </div><p className="text-xs text-muted-foreground">{[eq.brand, eq.model].filter(Boolean).join(' ')}</p></div>
          ...
          {filtered.length === 0 && search && <p className="text-sm text-muted-foreground text-center py-8">Geen apparatuur gevonden voor "{search}"</p>}
        </>
      )}
    </div>
  );
