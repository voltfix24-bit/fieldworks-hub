      ...
      <div className="animate-fade-in space-y-5">
        ...
        {overdueProjects.length > 0 && (
          ...
          >
            ...
            <ChevronRight className="h-4 w-4 text-destructive/25" />
          </button>
        )}
        ...
        <section>
          <div className="flex items-center justify-between mb-2">
            ...
            {projects && projects.length > 6 && (
              <button
                ...
                className="text-[12px] font-medium text-muted-foreground flex items-center gap-0.5 active:opacity-60"
              >
          ...
          {projects && projects.length > 0 ? (
            ...
            <div className="text-center py-12">
              <FolderKanban className="h-6 w-6 text-muted-foreground/15 mx-auto mb-2" />
        ...
        {projects && projects.length > 0 ? (
          <div className="rounded-2xl border border-border/60 bg-card overflow-hidden divide-y divide-border/40">
            {projects.slice(0, 8).map(p => (
              <div
                key={p.id}
                className="flex items-center justify-between py-3 px-4 hover:bg-muted/30 cursor-pointer transition-colors group"
          ...
          <p className="text-sm text-muted-foreground text-center py-8">Nog geen projecten</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex items-baseline gap-1.5">
      <span className={cn(
        ...
        alert ? 'text-destructive' : active ? 'text-foreground' : 'text-foreground/70'
      )}>{value}</span>
...
function DesktopStatCard({ title, value, sub, alert }: { title: string; value: number; sub?: string; alert?: boolean }) {
  return (
    <div className={cn(
      ...
      alert ? 'border-destructive/15' : 'border-border/50'
    )}>
      <p className="text-[12px] font-medium text-muted-foreground/60 mb-1">{title}</p>
      <p className={cn(
        'text-[28px] font-bold tracking-tight leading-none',
        alert ? 'text-destructive' : 'text-foreground'
      )}>{value}</p>
      {sub && <p className="text-[11px] text-muted-foreground/50 mt-1.5">{sub}</p>}
    </div>
  );
}
...
function SectionLabel({ children, noMargin }: { children: React.ReactNode; noMargin?: boolean }) {
  return (
    <p className={cn(
      'text-[11px] uppercase tracking-[0.08em] font-semibold text-muted-foreground/50',
      !noMargin && 'mb-2'
    )}>{children}</p>
  );
}
...
function ProjectRow({ project: p, onClick, showDate }: {
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        'w-full flex items-center gap-3 px-4 py-3 rounded-2xl bg-card transition-all text-left active:scale-[0.99]',
        today ? 'ring-1 ring-[hsl(var(--tenant-primary)/0.15)]' : ''
      )}
    >
