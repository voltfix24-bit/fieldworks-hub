    ...
    <Card>
      <CardHeader className="py-3 px-4">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-sm font-semibold">Schets & Bijlagen</CardTitle>
      ...
      <CardContent className="px-4 pb-4 pt-0 space-y-4">
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => { fileInputRef.current?.setAttribute('data-type', 'sketch_photo'); fileInputRef.current?.click(); }} disabled={uploading} className="flex-1">
              <ImageIcon className="mr-1.5 h-3.5 w-3.5" /> {uploading ? 'Uploaden…' : 'Schets Uploaden'}
            </Button>
            <Button variant="outline" size="sm" onClick={() => { fileInputRef.current?.setAttribute('data-type', 'other'); fileInputRef.current?.click(); }} disabled={uploading} className="flex-1">
          ...
          <div className="space-y-2">
            {[...sketches, ...otherAttachments].map((att: any) => (
              <div key={att.id} className="flex items-center gap-3 p-2 rounded-md border border-border bg-muted/20">
                {att.file_url && <img src={att.file_url} alt={att.file_name || 'Bijlage'} className="w-12 h-12 object-cover rounded" />}
                ...
                <Button size="sm" variant="ghost" onClick={() => deleteAttachment.mutate({ id: att.id, projectId })} className="h-7 w-7 p-0 text-muted-foreground hover:text-destructive shrink-0"><Trash2 className="h-3.5 w-3.5" /></Button>
              </div>
            ))}
          </div>
        )}
